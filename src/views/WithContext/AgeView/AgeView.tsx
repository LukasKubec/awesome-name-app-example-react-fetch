...
export const AgeView = (): JSX.Element => {
    ...
    if (response?.error)
        return <Typography variant="body1">Error: {response?.error.error}</Typography>;

    if (!!response?.data && response.success)
        return <Typography variant="body1">Your age is probably: {response?.data.age}</Typography>;
